...
{
    Elf64_Ehdr *p = (Elf64_Ehdr*)malloc(sizeof(Elf64_Ehdr));
    ReaderSetCursor(0, 0);
...
{
    Elf64_Shdr *p = (Elf64_Shdr*)malloc(sizeof(Elf64_Shdr));
...
static void FindContext()
{
    ...
    for (int i = 0; i < cnt; ++i) {
        ...
        if (p->sh_type == SHT_SYMTAB) {
            elf_shsymbol = (Elf64_Shdr*)malloc(sizeof(Elf64_Shdr));
            memcpy(elf_shsymbol, p, sizeof(Elf64_Shdr));
        } else if (p->sh_type == SHT_STRTAB) {
            strtab_list = (char*)malloc(sizeof(char)*p->sh_size);
...
static void FindFuncs()
{
    ...
    elf_func_syms = elf_func_cnt==0 ? NULL : (Elf64_Sym*)malloc(sizeof(Elf64_Sym)*elf_func_cnt);
    FOREACH_ELFSYM(sym) {
        if (ELF64_ST_TYPE(sym->elf_sym.st_info) == STT_FUNC) {
...
{
    symStr* sym = (symStr*)malloc(sizeof(symStr));
