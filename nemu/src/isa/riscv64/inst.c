...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + imm, 2)&BITMASK(16), 16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + imm, 2)&BITMASK(16));
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = SEXT(Mr(src1 + imm, 1)&BITMASK(8), 8));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 1)&BITMASK(8));
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + imm, 8, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2&BITMASK(32)));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2&BITMASK(16)));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, CsrFunc csr_ = CSR; if (dest != 0) R(dest) = csr_.read(&csr_); csr_.write(&csr_, src1));
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, CsrFunc csr_ = CSR; R(dest) = csr_.read(&csr_); csr_.write(&csr_, src1|R(dest)));
